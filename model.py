...
class LandmarkModel(object):
    ...
    def __call__(self, input_tensor):
        ...
        with tf.variable_scope('layer1'):
            ...
            conv1 = tf.layers.conv2d(inputs=inputs, filters=32, kernel_size=[3, 3], strides=(1, 1), padding='SAME',
        ...
        with tf.variable_scope('layer3'):
            conv4 = tf.layers.conv2d(inputs=pool2, filters=64, kernel_size=[3, 3], strides=(1, 1), padding='SAME',
                                     activation=tf.nn.relu)
            conv5 = tf.layers.conv2d(inputs=conv4, filters=64, kernel_size=[3, 3], strides=(1, 1), padding='SAME',
                                     activation=tf.nn.relu)
            pool3 = tf.layers.max_pooling2d(inputs=conv5, pool_size=[2, 2], strides=(2, 2), padding='SAME')

        # |== Layer 4 ==|
        with tf.variable_scope('layer4'):
            conv6 = tf.layers.conv2d(inputs=pool3, filters=128, kernel_size=[3, 3], strides=(1, 1), padding='SAME',
                                     activation=tf.nn.relu)
            conv7 = tf.layers.conv2d(inputs=conv6, filters=128, kernel_size=[3, 3], strides=(1, 1), padding='SAME',
                                     activation=tf.nn.relu)
            pool4 = tf.layers.max_pooling2d(inputs=conv7, pool_size=[2, 2], strides=(1, 1), padding='SAME')

        # |== Layer 5 ==|
        with tf.variable_scope('layer5'):
            conv8 = tf.layers.conv2d(inputs=pool4, filters=256, kernel_size=[3, 3], strides=(1, 1), padding='SAME',
                                     activation=tf.nn.relu)
        ...
        with tf.variable_scope('layer6'):
            ...
            logits = tf.layers.dense(inputs=dense1, units=self.output_size, activation=None, use_bias=True,
                                     name="logits")
